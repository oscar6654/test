...
describe Cohort do
  describe ".new" do
    it "takes a title, start_date, and end_date as arguments" do
      ...
      cohort = Cohort.new("Newbs.js", first_day, last_day)
      expect(cohort).to be_a(Cohort)
    end
  end

  let(:cohort) do
    Cohort.new("Newbs.js", Date.parse("2020/02/22"), Date.parse("2020/04/29"))
  end

  describe "#title" do
    it "has a reader for title" do
      expect(cohort.title).to eq("Newbs.js")
    end

    it "does not have a writer for title" do
      expect { cohort.title = "Newbs.js" }.to raise_error(NoMethodError)
    end
  end
  ...
  describe "#roster" do
    ...
    it "includes the cohort title" do
      expect(cohort_with_students.roster).to include("Newbs.js")
    end
