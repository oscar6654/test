...
#YOUR CODE GOES HERE
#puts twitter_data[0]["LaunchAcademy"]["description"]
def name_desc
  puts "Each username followed by its description. If there is no description (i.e., nil then print out 'NA' instead):"
  twitter_data.each do |data|
    data.keys.each do |names|
      if data[names]["description"].nil?
        puts "#{names}: NA"
      else
        puts "#{names}: #{data[names]["description"]}"
      end
    end
  end
end
...
def name_followers
  ...
  twitter_data.each do |data|
    data.keys.each do |names|
      puts "#{names}: #{data[names]["number of followers"]}"
    end
  end
end
...
def name_tweet
  puts "Each username and the length of their latest tweet."
  twitter_data.each do |data|
    data.keys.each do |names|
      puts "#{names}'s latest tweet was #{data[names]["latest tweet"].length} characters long."
    end
  end
end

def name_last_twenty
  puts "Each username followed by the total character count used in that user's last twenty tweets."
  twitter_data.each do |data|
    data.keys.each do |names|
      last_twenty = data[names]["last twenty tweets"]
      sum_of_twenty = 0
      last_twenty.each do |tweets|
        sum_of_twenty+= tweets.length
      end
      puts "#{names} used #{sum_of_twenty} characters in their last twenty tweets."
    end
  end
end
...
def most_followers
  ...
  twitter_data.each do |data|
    data.keys.each do |names|
      if data[names]["number of followers"] > top_follower
        top_follower = data[names]["number of followers"]
        top_name = names
      end
    end
  end
  puts "#{top_name} has the top followers with #{top_follower}"
end
...
def most_friends
  ...
  twitter_data.each do |data|
    data.keys.each do |names|
      if data[names]["number of friends"] > top_friends
        top_friends = data[names]["number of friends"]
        top_name = names
      end
    end
  end
  puts "#{top_name} has the top friends with #{top_friends}"
end
...
def most_tweets
  ...
  twitter_data.each do |data|
    data.keys.each do |names|
      if data[names]["number of tweets"] > top_tweets
        top_tweets = data[names]["number of tweets"]
        top_name = names
      end
    end
  end
  puts "#{top_name} has the most tweets with #{top_tweets}"
end
